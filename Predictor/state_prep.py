...
# =================================================================================================================
# This script reads in the raw state-level data, preprocess it, and trains and random forest regression on it
# =================================================================================================================
import numpy as np
import pandas as pd
...
def preprocessor():
    ...
    joblib.dump(lb_make, "data_maps/state_map.pkl")
    ...
    for row_idx in range(0, len(df)):
        ...
        # add the state code to all instances
    ...
    pd.DataFrame(X).to_csv("preprocessed_data/state_X.csv", index=False, header=False)
    pd.DataFrame(Y).to_csv("preprocessed_data/state_Y.csv", index=False, header=False)

# =================================================================================================================
def model_test():
    X = pd.read_csv("preprocessed_data/state_X.csv", header=None).values
    Y = pd.read_csv("preprocessed_data/state_Y.csv", header=None).values
    ...
    file_obj = open("tuning_results/state_level.csv", 'w')
...
def model_generate():
    X = pd.read_csv("preprocessed_data/state_X.csv", header=None).values
    Y = pd.read_csv("preprocessed_data/state_Y.csv", header=None).values
    model = RandomForestRegressor(n_estimators=200, max_depth=20, random_state=SEED, n_jobs=-1)
    model.fit(X, Y)
    joblib.dump(model, "models/state_model.pkl", compress=True)
