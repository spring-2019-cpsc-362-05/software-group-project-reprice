...
# =================================================================================================================
# This script reads in the raw county-level data, preprocess it, and trains and random forest regression model on it
# =================================================================================================================
import numpy as np
import pandas as pd
...
# =================================================================================================================
def preprocessor():
    df = pd.read_csv('data/County_MedianValuePerSqft_AllHomes.csv', encoding='latin-1')
    # drop extra columns
    df.drop(columns=['RegionID', 'State', 'Metro', 'SizeRank', 'StateCodeFIPS', 'MunicipalCodeFIPS'], inplace=True)

    # Encode the region name
    lb_make = LabelEncoder()
    df["RegionName"] = lb_make.fit_transform(df["RegionName"])
    joblib.dump(lb_make, "data_maps/county_map.pkl")
    ...
    for row_idx in range(0, len(df)):
        ...
        # add the county code to all instances
    ...
    pd.DataFrame(X).to_csv("preprocessed_data/county_X.csv", index=False, header=False)
    pd.DataFrame(Y).to_csv("preprocessed_data/county_Y.csv", index=False, header=False)

# =================================================================================================================
def model_test():
    X = pd.read_csv("preprocessed_data/county_X.csv", header=None).values
    Y = pd.read_csv("preprocessed_data/county_Y.csv", header=None).values
    # print(X.shape)
    # print(Y.shape)
    ...
    file_obj = open("tuning_results/county_level.csv", 'w')
...
def model_generate():
    ...
    model.fit(X, Y)
    joblib.dump(model, "models/county_model.pkl", compress=True)
